...
void LoginSession::DeleteChannelSession(const ChannelId &channelId)
{
    ...
    else
    {
        ...
        while(_channelSessions.Contains(channelId))
            WaitRemoveChannelSession(channelId);
    }
    return;
}

void LoginSession::WaitRemoveChannelSession(ChannelId channelId)
{
    FDateTime Then = FDateTime::Now() + FTimespan::FromSeconds(0.2);
    for (; Then > FDateTime::Now();) {
