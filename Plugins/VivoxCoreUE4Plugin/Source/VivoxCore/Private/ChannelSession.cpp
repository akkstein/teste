...
void ChannelSession::HandleEvent(const vx_evt_base_t& evt)
{
    ...
    case evt_participant_added:
    {
        ...
        if (ensure(!_participants.Contains(AccountId::AccountNameFromUri(tevt.participant_uri))))
        {
...
void ChannelSession::Disconnect()
{
        ...
        TextState() == ConnectionState::Connecting || TextState() == ConnectionState::Connected)
    {
        ...
        VivoxNativeSdk::Get().RemoveSession(_groupHandle, _sessionHandle, innerDelegate);
    }
